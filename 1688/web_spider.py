...
def get_cate():
    ...
    #  '女士内衣', '文胸内衣', '袜子', '內衣', '女士内裤', '内衣文胸', '胸罩', '收腹裤', '隐形文胸', '孕妇文胸', '卫裤', '地摊袜子', '儿童袜子', '平角内裤',
...
def get_detail(url):
    ...
    if os.path.exists('./5/' + '%s' % product_info['goods_id']) == False:
        os.makedirs('./5/' + '%s' % product_info['goods_id'] + '/product_img')
        os.chdir('C:/Users/admin/Desktop/1688/5/' + '%s' % product_info['goods_id'] + '/product_img')
        i = 1
        ...
        if videoId != '0':
            ...
            temp_video_url = re.search('address\"\:\"(.*?)\"', res2, re.S).group(1)
            os.chdir('C:/Users/admin/Desktop/1688/5/' + '%s' % product_info['goods_id'])
            os.mkdir('./product_video')
            os.chdir('C:/Users/admin/Desktop/1688/5/' + '%s' % product_info['goods_id'] + '/product_video')
            with open('%s' % product_info['goods_id'] + '.mp4', 'wb') as f:
                urllib3.disable_warnings(urllib3.exceptions.InsecureRequestWarning)
                video = requests.get(temp_video_url, verify=False).content
                f.write(video)
            if os.path.getsize(
                    'C:/Users/admin/Desktop/1688/5/' + '%s' % product_info['goods_id'] + '/product_video/' + '%s' %
                    product_info['goods_id'] + '.mp4') < 10000:
        ...
        try:
            ...
            for k, v in Specifications2.items():
                if "price" in v.keys():
                    price_dict[k.replace('gt;', '')] = {'price': v['price'], 'url': ''}
                else:
                    price_dict[k.replace('gt;', '')] = {'price': product_info['price'], 'url': ''}
            ...
            os.chdir(
                'C:/Users/admin/Desktop/1688/5/' + '%s' % product_info['goods_id'] + '/product_specifications/color')
            ...
            if len(Specifications1) >= 2:
                ...
                os.chdir(
                    'C:/Users/admin/Desktop/1688/5/' + '%s' % product_info['goods_id'] + '/product_specifications/size')
        ...
        saveHtmlFilePath = 'C:/Users/admin/Desktop/1688/5/' + '%s' % product_info['goods_id'] + '/' + '%s' % \
                           product_info[
                               'goods_id'] + '.html'
        # 参数依次为富文本模板，保存路径，图片列表
        format_html(file, saveHtmlFilePath, result)
        if '.' not in product_info['spcification_amount']:
            print(product_info)
