...
#             zone_list.append(city)
#     pprint.pprint(zone_list)
...
def get_cate():
    ...
    cate_json = json.loads(
        requests.get(cate_url, headers=cate_headers(), params=cate_params(), verify=False).text)
...
def get_shop_id(market_id):
    ...
    for cpage in range(1, math.ceil(int(shop_id_res['data']['total']) / 20) + 1):
...
def get_detail(shop_id):
    ...
    cursor = conn.cursor()
    try:
        sql = "INSERT INTO huo_yuan_copy1 (title, phone, wechat, cate) VALUE ('%s','%s','%s','%s')"
        base = (
