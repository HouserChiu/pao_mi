...
def get_result():
    ...
    data = {"n": "ShopList", "q": {"a": 0, "ta": {"pa": 1, "li": 12, "w": {"categoryId": 7}}},
            "s": "659f29cdcc84d8a2a7c85afd0c3ad9b8"}
    urllib3.disable_warnings(urllib3.exceptions.InsecureRequestWarning)
    res = json.loads(requests.post(result_url, headers=headers, verify=False, data=json.dumps(data)).text)
    for page in range(1, math.ceil(int(res['q']['total']) / 20) + 1):
        data_eve = {"n": "ShopList", "q": {"a": 0, "ta": {"pa": page, "li": 12, "w": {"categoryId": 7}}},
                    "s": "659f29cdcc84d8a2a7c85afd0c3ad9b8"}
        urllib3.disable_warnings(urllib3.exceptions.InsecureRequestWarning)
        res_temp = json.loads(requests.post(result_url, headers=headers, verify=False, data=json.dumps(data_eve)).text)

        for detail_res in res_temp['q']['shops']:
            store_info_dict = {}
